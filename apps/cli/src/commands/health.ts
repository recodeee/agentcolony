...
function readinessSummaryPayload(
  ...
  let queenStatus: ReadinessStatus = 'bad';
  if (queen.active_plans > 0) {
    queenStatus = readyUnclaimedPlan
      ? 'bad'
...
function queenWaveHealthPayload(
  ...
  for (const [planSlug, subtasks] of groupPlanSubtasks(readPlanSubtasks(storage, tasks))) {
    ...
    const readySubtasks = subtasks
      .filter((subtask) => isReadyPlanSubtask(subtask, subtasks))
